...
#ifdef __APPLE__
    #include <libproc.h>
    void clay_set_path(TCCState *s) {
        char pathbuf[4096];
        if (proc_pidpath(getpid(), pathbuf, 4096) <= 0) {
            fprintf(stderr, "pid_path(): %s\n", strerror(errno));
            exit(1);
        }
        char *basedir = strrchr(pathbuf,'/');
        if (basedir) *basedir = '\0';
        tcc_set_lib_path(s, pathbuf);
    }
#elif defined(__linux__)
    void clay_set_path(TCCState *s) {
        char pathbuf[4096];
        readlink( "/proc/self/exe", pathbuf, 4096);
        char *basedir = strrchr(pathbuf,'/');
        if (basedir) *basedir = '\0';
        tcc_set_lib_path(s, pathbuf);
    }
#else
    #define clay_set_path(s)
#endif

int clay_is_dir(const char* filename) {
...
redo:
    ...
    if ((n | t) == 0) {
        ...
        if (n == 0) {
            ...
            if (pdir) {
                ...
                while ((pent = readdir(pdir))) {
                    ...
                    sprintf(filename, "src/%s", pent->d_name);
                    const char *ext = tcc_fileextension(filename);
                    if (strcmp(ext, ".c") == 0 && !clay_is_dir(filename)) {
                        args_parser_add_file(s, filename, 0);
                    }
                }
                closedir(pdir);
            } else {
        ...
        if (s->output_type == TCC_OUTPUT_PREPROCESS) {
            ...
            if (n > 1 && s->outfile) tcc_error("cannot specify output file with -c many files");
        } else {
    ...
    tcc_delete(s);
    if (ret == 0 && n) goto redo; /* compile more files with -c */
